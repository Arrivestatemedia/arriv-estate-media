...
export const OFFER_NOT_EXTENDED_SUBJECT = "Update on Your Application – Arriv Sales Growth Advisor";

export function buildOfferNotExtendedHtml(firstName: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
export function buildOfferNotExtendedEmail(app: { full_name?: string; email?: string }) {
  const firstName = deriveFirstName(app.full_name);
  return {
    to: app.email,
    subject: OFFER_NOT_EXTENDED_SUBJECT,
    htmlContent: buildOfferNotExtendedHtml(firstName),
  };
}