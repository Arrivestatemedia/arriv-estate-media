...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch Cal Couch's activity
    const activities = await base44.asServiceRole.entities.ActivityLog.filter({
      contact_name: 'Cal Couch'
    }, '-activity_date', 1);

    if (!activities || activities.length === 0) {
      return Response.json({ error: 'Cal Couch activity not found' }, { status: 404 });
    }
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p style="color: #999; font-size: 12px;">— Arriv Estate Media</p>
    ...
    return Response.json({ success: true, email: repEmail, contact, time });
