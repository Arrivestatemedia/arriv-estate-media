...
// Default email content for templates whose HTML is defined inline in their
// sending functions (not in shared modules). Variables use {{var}} syntax.
const INLINE_DEFAULTS: Record<string, { subject: string; html: string }> = {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#FFFBF5;padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background-color:#FFFFFF;border-radius:14px;border:1px solid rgba(184,149,106,0.25);overflow:hidden;">
        ...
        <tr><td style="padding:40px 44px;">
          <h1 style="margin:0 0 8px;font-size:20px;font-weight:600;color:#1A1A1A;">Congratulations, {{firstName}}!</h1>
...
// Templates whose HTML is built by shared modules — call with placeholder vars.
function getSharedDefault(templateKey: string): { subject: string; html: string } | null {
  switch (templateKey) {
    case "sendApplicationWelcomeEmail":
      return { subject: "Thank you for applying to Arriv Estate Media", html: buildWelcomeHtml("{{firstName}}") };
    case "sendSalesInterviewScheduledEmail":
      return {
        subject: "Your Interview is Scheduled – Arriv Sales Growth Advisor",
        html: buildInterviewScheduledHtml("{{firstName}}", "{{scheduledDate}} {{scheduledTime}}", "{{durationMinutes}} minutes", "{{meetingLink}}"),
      };
    case "sendSalesOfferNotExtendedEmail":
      return { subject: "Update on Your Application – Arriv Sales Growth Advisor", html: buildOfferNotExtendedHtml("{{firstName}}") };
    default:
      return null;
  }
}
...
Deno.serve(async (req) => {
  try {
    ...
    // Check inline defaults first, then shared module defaults
    const inline = INLINE_DEFAULTS[templateKey];
    if (inline) {
      return Response.json({ subject: inline.subject, htmlBody: inline.html });
    }

    const shared = getSharedDefault(templateKey);
    if (shared) {
      return Response.json({ subject: shared.subject, htmlBody: shared.html });
    }

    // No default extracted yet
    return Response.json({
      subject: "",
      htmlBody: `<div style="padding:40px;text-align:center;font-family:sans-serif;color:#999;">
