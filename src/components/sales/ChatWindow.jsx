...
export default function ChatWindow({ chatType, chatId, chatName, currentUserId, currentUserName, currentUserEmail, memberProfiles = {}, memberStatuses = {}, onInitiateTransfer, onVideoCallStarted, onVideoCallEnded }) {
  ...
  const renderMessageContent = (content, isOutgoing = false) => {
    ...
    // Markdown link format so Arriv One's react-markdown renders "Connect conference
    // room" as a hyperlink. Estate Media's custom renderer parses the same format.
    const videoLinkMatch = content.match(/^(.+?click here to join: )\[Connect conference room\]\(([^)]+)\)$/);
    if (videoLinkMatch) {
      return (
                          ...
                          try {
                            ...
                            if (res.data?.success) {
                              ...
                              if (isArrivOneLinked && dmRecipient?.email) {
                                ...
                                const linkMessage = `${currentUserName} would like to have a Connect video conference with you, click here to join: [Connect conference room](${joinLink})`;
