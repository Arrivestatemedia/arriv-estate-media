...
export default function EditBookingDialog({ booking, open, onOpenChange, onSave }) {
  ...
  const handleDateSelect = (date) => {
    if (!date) return;
    // Extract local date components to ensure we capture what the user sees
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Calendar
              mode="single"
              selected={formData.preferred_date ? parse(formData.preferred_date, 'yyyy-MM-dd', new Date()) : undefined}
