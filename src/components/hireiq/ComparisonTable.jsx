...
export default function ComparisonTable({ candidates }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {candidates.map(c => (
          <label key={c.id} className="flex items-center gap-2 text-sm rounded-lg px-3 py-1.5 cursor-pointer transition-colors"
            style={{ backgroundColor: selected.includes(c.id) ? "#F5F2EC" : CREAM, border: "1px solid rgba(184,149,106,0.15)", color: TEXT_DARK }}>
      ...
      {toCompare.length === 0 ? (
        <p className="text-sm text-center py-8" style={{ color: MUTED_DARK }}>Select candidates to compare</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                <th className="p-2 text-left w-40" style={{ backgroundColor: "#F5F2EC", border: "1px solid rgba(184,149,106,0.12)", color: MUTED_DARK }}>Metric</th>
                {toCompare.map(c => (
                  <th key={c.id} className="p-2 text-center font-bold" style={{ backgroundColor: "#F5F2EC", border: "1px solid rgba(184,149,106,0.12)", color: TEXT_DARK }}>{c.name}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {FIELDS.map(field => (
                <tr key={field.key}>
                  <td className="p-2 font-medium" style={{ border: "1px solid rgba(184,149,106,0.12)", color: MUTED_DARK }}>{field.label}</td>
                  {toCompare.map(c => {
                    const val = c.evaluation?.[field.key];
                    return (
                      <td key={c.id} className="p-2 text-center align-top" style={{ border: "1px solid rgba(184,149,106,0.12)", color: TEXT_DARK }}>
                        {field.type === "score" && val != null ? (
                          ...
                          <span className="text-xs" style={{ color: TEXT_DARK }}>{val || "—"}</span>
                        )}
                      </td>
                    );
                  })}
