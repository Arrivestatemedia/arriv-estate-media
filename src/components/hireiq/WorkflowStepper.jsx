            ...
            <React.Fragment key={step.num}>
              {i > 0 && (
                <div className="flex-1 h-px mt-3" style={{
                  backgroundColor: isComplete ? "#B8956A" : "rgba(255,251,245,0.1)",
                }} />
              )}
              <div className="flex flex-col items-center" style={{ minWidth: "52px" }}>
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold transition-all"
                  style={{
                    backgroundColor: isComplete ? "#B8956A" : isCurrent ? "rgba(255,251,245,0.05)" : "transparent",
                    border: `1.5px solid ${isComplete ? "#B8956A" : isCurrent ? "#B8956A" : "rgba(255,251,245,0.15)"}`,
                    color: isComplete ? "#1A1A1A" : isCurrent ? "#B8956A" : "rgba(255,251,245,0.3)",
                    boxShadow: isCurrent ? "0 0 12px rgba(184,149,106,0.3)" : "none",
                  }}>
                  {step.num}
                </div>
                <span className="text-[10px] mt-1.5 font-medium whitespace-nowrap" style={{
                  color: isCurrent ? "#B8956A" : isComplete ? "#FFFBF5" : "rgba(255,251,245,0.3)",
                }}>{step.label}</span>
              </div>
            </React.Fragment>
          );
