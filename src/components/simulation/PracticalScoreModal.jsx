...
export default function PracticalScoreModal({ open, onClose, practicalType, learnerName, onSubmit }) {
  ...
  const handleSubmit = () => {
    onSubmit({
      ...
      score: totalScore,
      critical_failures: selectedFailures,
      notes,
    });
    setScores({});
    setSelectedFailures([]);
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
          ...
          {rubric.categories.map(cat => (
            <div key={cat.key} className="flex items-center gap-3">
              ...
              <input
                ...
                className="w-20 px-2 py-1.5 text-sm border border-[#B8956A]/30 rounded-lg text-center focus:outline-none focus:border-[#B8956A]"
                placeholder="0"
              />
            </div>
          ))}
          ...
          <div className="space-y-1 max-h-40 overflow-y-auto border border-[#B8956A]/20 rounded-lg p-2">
            {CRITICAL_FAILURES.map(cf => (
              <label key={cf.key} className="flex items-start gap-2 p-1.5 rounded hover:bg-[#1A1A1A]/5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={selectedFailures.includes(cf.key)}
                  onChange={() => toggleFailure(cf.key)}
                  className="mt-0.5"
                />
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            placeholder="Coaching feedback for the learner..."
          />
        </div>
