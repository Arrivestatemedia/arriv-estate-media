...
function SimulationContent() {
  ...
  const [view, setView] = useState("levels"); // "levels" | "modules" | "manager"
  const [selectedLevel, setSelectedLevel] = useState("WATCH_IT");
  const [selectedModule, setSelectedModule] = useState("E4");
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      {/* View toggle: Levels | Module Practicals | Manager Review */}
      <div className="flex gap-2 flex-wrap">
        ...
        <button onClick={() => setView("modules")} className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-1.5 ${view === "modules" ? "bg-[#1A1A1A] text-[#FFFBF5]" : "bg-white border border-[#B8956A]/20 text-[#1A1A1A] hover:border-[#B8956A]"}`}>
          <BookOpen className="w-4 h-4" /> Module Practicals (E4–E19)
        </button>
        {isAdmin && (
      ...
      {view === "modules" && (
        <>
          {/* Module selector */}
          <div className="flex gap-2 overflow-x-auto pb-1">
            {getModulesWithPracticals().map(modId => (
              <button key={modId} onClick={() => setSelectedModule(modId)}
                className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${selectedModule === modId ? "bg-[#1A1A1A] text-[#FFFBF5]" : "bg-white border border-[#B8956A]/20 text-[#1A1A1A] hover:border-[#B8956A]"}`}>
                {modId}
              </button>
            ))}
          </div>
          {(() => {
            const mod = MODULE_PRACTICALS[selectedModule];
            if (!mod) return null;
            return (
              <>
                <div className="p-4 rounded-lg bg-[#B8956A]/10 border border-[#B8956A]/20">
                  <h2 className="font-serif text-lg text-[#1A1A1A]">{mod.module_id}: {mod.title}</h2>
                  <p className="text-xs text-[#B8956A] mt-1">{mod.chapter}</p>
                  <p className="text-sm text-[#1A1A1A]/70 mt-2">{mod.core_flow}</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {PRACTICAL_TYPES.map(pt => {
                    const practical = mod[pt.id];
                    if (!practical) return null;
                    const Icon = PRACTICAL_ICONS[pt.id] || Play;
                    const scenario = practical.scenario_id ? getScenarioById(practical.scenario_id) : null;
                    return (
                      <div key={pt.id} className={`p-4 rounded-xl border ${scenario ? "border-[#B8956A]/20 bg-white hover:border-[#B8956A] hover:bg-[#B8956A]/5 transition-all" : "border-[#1A1A1A]/10 bg-[#1A1A1A]/5"} group ${scenario ? "cursor-pointer" : "cursor-default"}`}
                        onClick={() => scenario && startScenario(scenario.id, scenario.level)}>
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <Icon className="w-4 h-4 text-[#B8956A]" />
                            <h3 className="font-medium text-[#1A1A1A]">{practical.label || pt.label}</h3>
                          </div>
                          {scenario && <Play className="w-4 h-4 text-[#B8956A] group-hover:scale-110 transition-transform flex-shrink-0 mt-0.5" />}
                        </div>
                        <p className="text-sm text-[#1A1A1A]/60 mb-2">{practical.description}</p>
                        {scenario ? (
                          <div className="flex items-center gap-2 text-xs">
                            <span className={`px-2 py-0.5 rounded-full font-medium ${scenario.criticality === "critical" ? "bg-red-100 text-red-700" : scenario.criticality === "high" ? "bg-amber-100 text-amber-700" : "bg-[#1A1A1A]/5 text-[#1A1A1A]/60"}`}>{scenario.criticality}</span>
                            <span className="text-[#1A1A1A]/40">{scenario.steps.length} steps</span>
                            <span className="text-[#B8956A]">{scenario.level.replace("_", " ")}</span>
                          </div>
                        ) : (
                          <p className="text-xs text-[#1A1A1A]/40 italic">Onboarding practical — no simulation scenario</p>
                        )}
                      </div>
                    );
                  })}
