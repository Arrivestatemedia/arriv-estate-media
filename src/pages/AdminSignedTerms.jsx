...
export default function AdminSignedTerms() {
  ...
  const { data: signedTerms = [], isLoading } = useQuery({
    queryKey: ["signed-terms"],
    queryFn: () => base44.entities.SignedTerms.list("-signed_date", 100),
  });

  const handleDownload = async (fileUri, mediaPartnerName) => {
    try {
      ...
      link.download = `${mediaPartnerName}_signed_terms.pdf`;
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-[var(--text-primary)]">
            Signed Terms Documents
          </h1>
          <p className="text-[var(--text-secondary)] mt-1">
            View and download media partner signed terms agreements
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-[var(--text-secondary)]">No signed terms documents yet.</p>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-lg text-[var(--text-primary)]">
                          {record.media_partner_name}
                        </CardTitle>
                        <p className="text-sm text-[var(--text-secondary)]">
                          {record.media_partner_email}
                        </p>
                      </div>
                    </div>
                    <Button
                      onClick={() =>
                        handleDownload(record.document_url, record.media_partner_name)
                      }
