...
export default function SalesTrainingPortal() {
  ...
  const [view, setView] = useState('list'); // list | video | quiz
  ...
  if (!memberId) {
    return <div className="min-h-screen flex items-center justify-center p-8"><Card className="p-8 text-center"><p className="text-slate-600">Please log in to access the training portal.</p></Card></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF5] p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <Card className="p-6 mb-6 bg-[#1A1A1A] border-[#B8956A]/20">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <Award className="w-6 h-6 text-[#B8956A]" />
                <span className="text-lg font-semibold text-[#FFFBF5]">Certification Status</span>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="text-2xl font-bold text-[#B8956A]">{cert.modules_passed_count || 0}</p>
                <p className="text-xs text-[#FFFBF5]/60">Modules Passed</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-[#B8956A]">{cert.quiz_average_score ? Math.round(cert.quiz_average_score) : 0}%</p>
                <p className="text-xs text-[#FFFBF5]/60">Quiz Average</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-[#B8956A]">{cert.final_exam_passed ? "✓" : "—"}</p>
          ...
          {(cert.critical_failures || []).length > 0 && (
            <div className="mt-4 p-3 bg-red-500/10 rounded-lg flex items-start gap-2">
              ...
              <div>
                <p className="text-sm font-medium text-red-300">Critical Failures Active</p>
                <p className="text-xs text-red-300/80">{cert.critical_failures.join(", ").replace(/_/g, " ")}</p>
              </div>
            </div>
          )}
        </Card>
        ...
        {view === 'list' && (
          <div className="space-y-3">
            {modules.map((mod, index) => {
              ...
              return (
                <Card key={mod.module_id} className={`p-4 ${!unlocked ? 'opacity-60' : ''} bg-white border-[#B8956A]/15 hover:border-[#B8956A]/40 transition-colors`}>
                  <div className="flex items-center gap-4">
                    <div className="flex-shrink-0">
                      {quizPassed ? <CheckCircle2 className="w-8 h-8 text-[#B8956A]" /> :
                       unlocked ? <PlayCircle className="w-8 h-8 text-slate-400" /> :
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        {watchComplete !== undefined && (
                          <Badge variant="outline" className="text-xs">
                            ...
                            {Math.round(prog.completion_percentage)}% watched
                          </Badge>
                        )}
                        {bestAttempt && (
                    ...
                    {unlocked && (
                      <Button
                        size="sm"
                        className="bg-[#B8956A] hover:bg-[#A68559] text-[#1A1A1A] shrink-0"
        ...
        {view === 'quiz' && selectedModule && (
          <QuizInterface
            ...
            memberName={memberName}
            memberEmail={memberEmail}
            previousAttempts={getModuleAttempts(selectedModule.module_id)}
            certification={cert}
...
// ─── Video Player with Seek Detection ──────────────────────────────────────
function VideoPlayer({ module, memberId, memberName, memberEmail, progress, onWatchComplete, onBack }) {
  ...
  useEffect(() => {
    if (progress?.watch_segments) {
      watchedSegmentsRef.current = progress.watch_segments;
    }
  }, [progress]);
  ...
  const calculateUniqueWatched = () => {
    return watchedSegmentsRef.current.reduce((sum, seg) => sum + (seg.end - seg.start), 0);
  };
  ...
  const handleTimeUpdate = () => {
    ...
    if (current > last && current - last < 2) {
      addWatchedSegment(last, current);
    }
  ...
  const saveProgress = async (completed, pct) => {
    ...
    try {
      ...
      const data = {
        ...
        seeking_detected: false,
        watch_segments: watchedSegmentsRef.current,
        last_updated_at: now,
        completed_at: completed ? now : undefined,
      };
      if (progress?.id) {
        await base44.entities.VideoWatchProgress.update(progress.id, data);
      } else {
        await base44.entities.VideoWatchProgress.create({ ...data, started_at: now });
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-6 bg-white border-[#B8956A]/15">
        <h2 className="text-xl font-bold text-[#1A1A1A] mb-2">{module.title}</h2>
        ...
        {module.video_url ? (
          ...
          <div className="w-full h-64 bg-slate-100 rounded-lg flex items-center justify-center">
            <p className="text-slate-400">Video not yet uploaded for this module</p>
          </div>
        )}
        <div className="mt-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-slate-600">Watch Progress</span>
            <span className="text-sm font-bold text-[#B8956A]">{Math.round(completionPct)}%</span>
          </div>
          <Progress value={completionPct} className="h-2" />
          <p className="text-xs text-slate-500 mt-2">
            Requires {CERTIFICATION_REQUIREMENTS.min_watch_percentage}% watch completion. Seeking to the end will not count.
          </p>
          {isComplete && (
            <div className="mt-4 p-3 bg-[#B8956A]/10 rounded-lg flex items-center gap-2">
              <CheckCircle2 className="w-5 h-5 text-[#B8956A]" />
              <span className="text-sm font-medium text-[#1A1A1A]">Watch complete! You can now take the quiz.</span>
            </div>
          )}
          {isComplete && (
            <Button className="mt-4 bg-[#B8956A] hover:bg-[#A68559] text-[#1A1A1A]" onClick={onWatchComplete}>
...
function QuizInterface({ module, memberId, memberName, memberEmail, previousAttempts, certification, onQuizComplete, onBack }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const attempt = await base44.entities.TrainingAttempt.create({
        ...
        all_critical_correct: allCriticalCorrect,
        question_set_snapshot: questions,
      ...
      await base44.entities.AuditEvent.create({
        ...
        details: { module_id: module.module_id, score: Math.round(score * 10) / 10, passed },
        timestamp: now,
      });
  ...
  if (result) {
    return (
      <div className="space-y-4">
        <Card className={`p-8 text-center ${result.passed ? 'bg-[#B8956A]/10' : 'bg-red-50'}`}>
          {result.passed ? <CheckCircle2 className="w-16 h-16 text-[#B8956A] mx-auto mb-4" /> : <AlertTriangle className="w-16 h-16 text-red-500 mx-auto mb-4" />}
          <h2 className="text-2xl font-bold text-[#1A1A1A] mb-2">{result.passed ? "Quiz Passed!" : "Quiz Not Passed"}</h2>
          <p className="text-lg text-slate-600 mb-4">Score: {result.score}% ({result.correct}/{result.total} correct)</p>
          {result.criticalTotal > 0 && (
            <p className="text-sm text-slate-500 mb-4">Critical Questions: {result.criticalCorrect}/{result.criticalTotal} correct</p>
          )}
          {!result.passed && (
            <p className="text-sm text-red-600 mb-4">
              {result.criticalCorrect < result.criticalTotal
                ? "You must answer all critical questions correctly."
                : `Score must be at least ${CERTIFICATION_REQUIREMENTS.module_quiz_min_score}%.`}
            </p>
          )}
          <Button className="bg-[#B8956A] hover:bg-[#A68559] text-[#1A1A1A]" onClick={onQuizComplete}>
            {result.passed ? "Continue" : "Back to Modules"}
          </Button>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-6 bg-white border-[#B8956A]/15">
        <h2 className="text-xl font-bold text-[#1A1A1A] mb-1">{module.title} — Quiz</h2>
        <p className="text-sm text-slate-500 mb-6">Passing score: {CERTIFICATION_REQUIREMENTS.module_quiz_min_score}% • All critical questions must be correct</p>
            ...
            <div key={qi} className="space-y-3">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  {q.is_critical && <Badge className="mt-1 bg-red-100 text-red-700 text-xs">Critical Question</Badge>}
                </div>
              </div>
              <div className="space-y-2 ml-6">
                {q.choices.map((choice, ci) => (
                  <label key={ci} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${answers[qi] === ci ? 'border-[#B8956A] bg-[#B8956A]/5' : 'border-slate-200 hover:border-slate-300'}`}>
